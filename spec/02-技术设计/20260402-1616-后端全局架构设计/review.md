...
- **审查日期**: 2026-04-04
- **审查对象**: [[plan|plan.md]]
- **Spec 路径**: `spec/02-技术设计/20260402-1616-后端全局架构设计/`
- **审查模式**: 严格模式
...
> [!success] 本 Spec 声明范围止于「脚手架 + Schema + 路由骨架」；业务逻辑由后续 03-能力交付 各 Spec 承接，与 [[summary|summary.md]] 表述一致。

### 2.2 一致性 ⚠️

| 项 | Spec 定义 | 实际演进 | 说明 |
|----|-----------|----------|------|
| 密码哈希 | plan §2.2、§5.2：`passlib` | 项目规范与认证模块使用 `bcrypt` 直接调用 | ⚠️ 以 [[../../03-能力交付/20260403-1430-用户认证模块/summary|用户认证总结]] 与 `CLAUDE.md` 为准 |
